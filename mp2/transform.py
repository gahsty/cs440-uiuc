...
def transformToMaze(alien, goals, walls, window, granularity):
    ...
    """
    length = int(window[0] / granularity + 1)
    width = int(window[1] / granularity + 1)
    temp = [[['%' for k in range(3)] for j in range(width)] for i in range(length)]
    print(length,width)
    start = util.configToIdx(alien.get_config(), (0, 0, 0,), granularity, alien)
    for k in range(3):
        for i in range(length-1):
            for j in range(width-1):
                config = util.idxToConfig((i, j, k), (0, 0, 0), granularity, alien)
                alien.set_alien_config(config)
                Ju1 = geometry.does_alien_touch_goal(alien,goals)
                Ju2 = geometry.does_alien_touch_wall(alien,walls,granularity)
                Ju3 = geometry.is_alien_within_window(alien,window,granularity)
                if Ju2| (not Ju3):
                    temp[i][j][k] = '%'
                elif Ju1:
                    temp[i][j][k] = '.'
    ...
    temp[start[0]][start[1]][start[2]] = 'P'


    m = Maze(temp, alien, granularity)
    return m
